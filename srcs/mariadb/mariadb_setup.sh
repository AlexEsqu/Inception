...
echo "Secrets loaded successfully : ${MARIADB_NAME} ${MARIADB_USER} ${DB_ROOT_PASSWORD} ${DB_PASSWORD}"
...
then
	...
	echo "CREATE DATABASE ${MARIADB_NAME};" >> ${TMP}
	echo "CREATE USER '${MARIADB_USER}'@'%' IDENTIFIED BY '${DB_PASSWORD}';" >> ${TMP}
	echo "GRANT ALL PRIVILEGES ON ${MARIADB_NAME}.* TO '${MARIADB_USER}'@'%' IDENTIFIED BY '${DB_PASSWORD}';" >> ${TMP}
	echo "FLUSH PRIVILEGES;" >> ${TMP}
